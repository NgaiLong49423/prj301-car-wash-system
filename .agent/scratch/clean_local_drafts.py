...
# Delete all existing .md files to prevent duplicates
...
print("Updating local drafts and index...")

index_content = "# Mục lục Grouped Issues Đề xuất\n\n"
index_content += "| Group ID | Title | File Draft | FR-AS Cover | Issue Cũ | GitHub |\n"
index_content += "|---|---|---|---|---|---|\n"
...
"""
    ...
    filename = f"{gi['id']}-{slug}.md"
    filepath = os.path.join(OUTPUT_DIR, filename)
    
    with open(filepath, "w", encoding="utf-8") as f:
        f.write(body_content)
        
    index_content += f"| {gi['id']} | {gi['title']} | `{filename}` | {', '.join(gi['frs'])} | {gi['old_mapping']} | #{gi['gh_issue']} |\n"

with open(INDEX_PATH, "w", encoding="utf-8") as idx_f:
    idx_f.write(index_content)

print("Local drafts cleaned and regenerated.")
