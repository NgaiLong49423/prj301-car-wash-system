...
## [#64] Luồng đi dữ liệu của booking
- **URL**: https://github.com/NgaiLong49423/prj301-car-wash-system/issues/64
- **State**: open
- **Created At**: 2026-06-15T03:48:35Z
- **Labels**: `✨ Feature`, `⚙️ Backend`, `size-M`

### Description
<img width="2560" height="1920" alt="Image" src="https://github.com/user-attachments/assets/6fe6a277-d440-40fa-a5a5-c66dbf144d3d" />

Thiết kiết luồng đi của dữ liệu booking chia 3 luồng
* là khách hàng đã đặt 1 booking thì đang từ pendis
1. từ pendis thì khác hàng hủy booking thì luồng kết thúc
2. từ pendis được adim vô cheking bằng nút bấm. thì booking được chuyển sang checkout cho thời gian cho khách hàng vô rửa xong thì luồng kết thúc
3. từ pendis sau một thời gian ko được adim xác nhận hoặc khách hàng nhập form sai gì đấy thì đc đẩy sang luồn overtime checking là bị miss thì luồng kết thúc

---

## [#63] [Refactor] Replace hardcoded JSP data with database-driven data
- **URL**: https://github.com/NgaiLong49423/prj301-car-wash-system/issues/63
- **State**: open
- **Created At**: 2026-06-08T13:08:55Z
- **Labels**: `🟡 priority-medium`, `size-S`, `estimate-2`, `🔧 Refactor`

### Description
## Mô tả
Hiện tại một số dữ liệu hiển thị trên file JSP đang được hardcode trực tiếp trong giao diện. Điều này làm dữ liệu không phản ánh đúng database thật và khó bảo trì khi cần thay đổi.

Cần nâng cấp luồng xử lý để dữ liệu được lấy từ database thông qua Servlet và DAO, sau đó truyền sang JSP bằng request/session attribute.

## Vấn đề hiện tại
- Dữ liệu đang được viết cố định trong JSP.
- Khi database thay đổi, giao diện không tự cập nhật.
- Khó mở rộng và khó kiểm thử.
- Không đúng mô hình MVC2 vì JSP đang chứa dữ liệu tĩnh thay vì chỉ hiển thị dữ liệu.

## Yêu cầu thay đổi
- Tạo/cập nhật DAO để lấy dữ liệu từ database.
- Servlet gọi DAO để truy vấn dữ liệu thật.
- Servlet dùng `request.setAttribute()` để gửi dữ liệu sang JSP.
- JSP dùng JSTL/EL để hiển thị dữ liệu động.
- Loại bỏ dữ liệu hardcode trong JSP.

## Acceptance Criteria
- [ ] Không còn dữ liệu hardcode trong JSP.
- [ ] Dữ liệu hiển thị được lấy từ database thật.
- [ ] Servlet xử lý logic lấy dữ liệu.
- [ ] DAO chịu trách nhiệm truy vấn database.
- [ ] JSP chỉ dùng để render dữ liệu.
- [ ] Khi dữ liệu trong database thay đổi, giao diện cũng thay đổi theo.

## Gợi ý kỹ thuật
Luồng đúng nên là:

Browser → MainController → Servlet con → DAO → Database → Servlet → JSP → Browser

---

## [#62] [BUG] Kiểm tra biển số xe cho phép định dạng không hợp lệ
- **URL**: https://github.com/NgaiLong49423/prj301-car-wash-system/issues/62
- **State**: open
- **Created At**: 2026-06-06T09:50:11Z
- **Labels**: `🐞 Bug`, `size-XS`, `estimate-1`, `ui/ux`

### Description
## Current Behavior

Hiện tại hệ thống chỉ kiểm tra biển số xe có đủ 8 số là hợp lệ.

Ví dụ các giá trị như:
12345678

định dạng mong muốn qua phải là:
51A-123.45

<img width="517" height="412" alt="Image" src="https://github.com/user-attachments/assets/10510129-56f9-4cd6-bb18-3f2929459d95" />

---

## [#61] [BUG] Hero car image is not displayed on dashboards.jsp
- **URL**: https://github.com/NgaiLong49423/prj301-car-wash-system/issues/61
- **State**: open
- **Created At**: 2026-06-06T08:03:20Z
- **Labels**: `🐞 Bug`, `size-XS`, `estimate-1`, `ui/ux`

### Description
## Current Behavior

Khi người dùng đăng nhập và truy cập trang `dashboards.jsp`, ảnh hero car không hiển thị.

Thay vì hiển thị ảnh, giao diện chỉ hiện text alt:

<img width="1308" height="629" alt="Image" src="https://github.com/user-attachments/assets/f39e6ef9-15f4-4a24-b290-fb9c06c808f0" />

---

## [#60] [BUG] Truy cập trực tiếp rewards.jsp hiển thị thông tin Guest thay vì người dùng đã đăng nhập
- **URL**: https://github.com/NgaiLong49423/prj301-car-wash-system/issues/60
- **State**: open
- **Created At**: 2026-06-06T07:41:15Z
- **Labels**: `🐞 Bug`, `🎨 Frontend`, `⚙️ Backend`, `🔒 Security`, `size-XS`

### Description
## Current Behavior

Khi người dùng đã đăng nhập nhưng truy cập trực tiếp:

http://localhost:8080/rewwds.jsp

hệ thống không lấy được thông tin session hiện tại.

Trang rewards.jsp hiển thị dữ liệu Guest thay vì dữ liệu của người dùng đã đăng nhập.

---

## Expected Behavior

Trang rewards.jsp phải nhận được session của người dùng đã đăng nhập và hiển thị đúng thông tin khách hàng.
...
2. Không đi qua menu điều hướng
3. Nhập trực tiếp URL:

http://localhost:8080/rewwds.jsp

4. Quan sát dữ liệu hiển thị

---

## Actual Result

Hiển thị dữ liệu Guest.

---

## Suspected Cause

Có khả năng luồng truy cập trực tiếp vào JSP đã bỏ qua Controller hoặc Servlet chịu trách nhiệm load thông tin session và customer profile.
...
Phát hiện trước Workshop 2.
Chưa thực hiện fix.

---

## [#59] FR-14b AI Recommendations: Tích hợp hệ thống AI gợi ý khuyến mãi cá nhân hóa
- **URL**: https://github.com/NgaiLong49423/prj301-car-wash-system/issues/59
- **State**: open
- **Created At**: 2026-06-04T15:31:37Z
- **Labels**: `✨ Feature`, `⚙️ Backend`, `🟢 priority-low`, `size-L`, `estimate-8`

### Description
## Mục tiêu

Tích hợp chức năng AI (mô phỏng hoặc thực tế) nhằm đề xuất các chương trình khuyến mãi và ưu đãi được cá nhân hóa cho từng khách hàng dựa trên hành vi và lịch sử sử dụng dịch vụ của họ.

## Căn cứ yêu cầu

Issue này được xác định dựa trên:
- Tài liệu SRS.md (FR-14b AI Recommendations)
- File đề bài cô giáo giao (AI-powered customer engagement and personalization)
- Nhu cầu triển khai thực tế của project AutoWashCar

## Phạm vi công việc

- Nghiên cứu cơ chế gợi ý cá nhân hóa dựa trên lịch sử đặt lịch và hạng thành viên (ví dụ: mô phỏng thuật toán AI đơn giản bằng luật dựa trên tập luật nếu-thì, hoặc sử dụng API ngoài).
- Xây dựng module gợi ý chương trình khuyến mãi tương ứng cho từng khách hàng trên trang Dashboard của họ.
- Cho phép Admin xem hiệu quả của các gợi ý AI này đối với doanh số.

## Kết quả mong đợi

Sau khi hoàn thành issue này, hệ thống/tài liệu cần đạt được:

- Khách hàng nhận được các đề xuất khuyến mãi phù hợp trên giao diện khi đăng nhập.
- Module gợi ý chạy độc lập, dễ dàng nâng cấp thuật toán AI trong tương lai.

## Tiêu chí hoàn thành

- [ ] Hoàn thành đúng nội dung yêu cầu
- [ ] Có kiểm tra lại với SRS.md
- [ ] Không làm sai lệch yêu cầu từ đề bài cô giao
- [ ] Nội dung rõ ràng, dễ hiểu cho thành viên nhóm
- [ ] Sẵn sàng để review hoặc triển khai

## Requirement Mapping

### SRS Reference

- FR-14 Targeted Promotions
- FR-14b AI Recommendations

## Gợi ý kỹ thuật / ghi chú

- Đây là tính năng mở rộng trong tương lai (Future Enhancement / Optional Feature), ưu tiên triển khai sau khi hoàn tất các chức năng cốt lõi của đồ án.

---

## [#58] FR-10b — Tự động kích hoạt xét lại hạng khi dữ liệu loyalty thay đổi
- **URL**: https://github.com/NgaiLong49423/prj301-car-wash-system/issues/58
- **State**: open
- **Created At**: 2026-06-04T15:31:35Z
- **Labels**: `✨ Feature`, `⚙️ Backend`, `🟡 priority-medium`, `size-S`, `estimate-2`
...
## [#56] NFR-08 Usability: Giao diện người dùng đơn giản và phản hồi linh hoạt (Responsive)
- **URL**: https://github.com/NgaiLong49423/prj301-car-wash-system/issues/56
- **State**: open
- **Created At**: 2026-06-04T08:09:18Z
- **Labels**: `🎨 Frontend`, `🟡 priority-medium`, `size-M`, `estimate-3`, `requirement`, `non-functional`, `ui/ux`

### Description
## Mục tiêu

Đảm bảo giao diện người dùng hiển thị trực quan, dễ dùng và tương thích hoàn toàn trên cả PC, Tablet và thiết bị di động.

## Căn cứ yêu cầu

Issue này được xác định dựa trên:
- Tài liệu SRS.md (NFR-08 Usability)
- File đề bài cô giáo giao (Customer Profile - view, Customer Booking, Admin controls)
- Nhu cầu triển khai thực tế của project AutoWashCar

## Phạm vi công việc

- Áp dụng các kỹ thuật thiết kế Responsive Web Design bằng Vanilla CSS.
- Đơn giản hóa biểu mẫu đặt lịch và các trang thông tin cá nhân.
- Kiểm thử trải nghiệm giao diện trên Chrome DevTools mô phỏng di động.

## Kết quả mong đợi

Sau khi hoàn thành issue này, hệ thống/tài liệu cần đạt được:

- Layout tự động co giãn và hiển thị đẹp mắt trên màn hình di động, máy tính bảng.
- Dễ dàng thao tác và điều hướng cho cả Khách hàng và Admin.
...
## [#55] NFR-07 Security: Mã hóa mật khẩu (BCrypt) và phân quyền truy cập (RBAC)
- **URL**: https://github.com/NgaiLong49423/prj301-car-wash-system/issues/55
- **State**: open
- **Created At**: 2026-06-04T08:09:05Z
- **Labels**: `⚙️ Backend`, `🔒 Security`, `🔴 priority-high`, `size-M`, `estimate-3`, `requirement`, `non-functional`

### Description
## Mục tiêu

Đảm bảo tính bảo mật cho thông tin tài khoản của khách hàng bằng cách sử dụng giải pháp băm mật khẩu hiện đại và thiết lập bộ lọc phân quyền nghiêm ngặt.

## Căn cứ yêu cầu

Issue này được xác định dựa trên:
- Tài liệu SRS.md (NFR-07 Security)
- File đề bài cô giáo giao (Role-based Access)
- Nhu cầu triển khai thực tế của project AutoWashCar

## Phạm vi công việc

- Tích hợp giải pháp băm mật khẩu an toàn BCrypt (không dùng MD5) vào backend của dự án trước khi lưu trữ vào SQL Server.
- Triển khai Role-Based Access Control (RBAC) thông qua AuthFilter để bảo vệ các Servlet/URL của Admin và Customer.
- Phòng chống các lỗ hổng cơ bản như SQL Injection.

## Kết quả mong đợi

Sau khi hoàn thành issue này, hệ thống/tài liệu cần đạt được:

- Mật khẩu lưu trữ trong database được mã hóa băm an toàn.
- Ngăn chặn triệt để việc truy cập trái phép từ tài khoản thường vào trang quản lý Admin.
...
## [#54] NFR-06 Backup and Recovery: Sao lưu dữ liệu hàng ngày và khôi phục dưới 2 giờ
- **URL**: https://github.com/NgaiLong49423/prj301-car-wash-system/issues/54
- **State**: open
- **Created At**: 2026-06-04T08:08:54Z
- **Labels**: `🗄️ Database`, `🟡 priority-medium`, `size-S`, `estimate-2`, `requirement`, `non-functional`

### Description
## Mục tiêu

Thực hiện chính sách sao lưu tự động hàng ngày và đảm bảo thời gian khôi phục hệ thống (RTO) dưới 2 giờ khi xảy ra thảm họa.

## Căn cứ yêu cầu

Issue này được xác định dựa trên:
- Tài liệu SRS.md (NFR-06 Backup and Recovery)
- File đề bài cô giáo giao (NFR-06: Data Backup)
- Nhu cầu triển khai thực tế của project AutoWashCar

## Phạm vi công việc

- Tạo script tự động sao lưu SQL Server hàng ngày.
- Soạn thảo quy trình khôi phục cơ sở dữ liệu chi tiết khi có sự cố.
- Kiểm thử và diễn tập khôi phục DB nhằm đảm bảo thời gian RTO đạt yêu cầu.

## Kết quả mong đợi

Sau khi hoàn thành issue này, hệ thống/tài liệu cần đạt được:

- Các bản sao lưu được lưu trữ an toàn, hệ thống có khả năng phục hồi hoàn toàn trong vòng dưới 2 giờ.
...
- NFR-06 Backup and Recovery

---

## [#53] NFR-05 Availability: Đảm bảo thời gian hoạt động hệ thống đạt 99.5%
- **URL**: https://github.com/NgaiLong49423/prj301-car-wash-system/issues/53
- **State**: open
- **Created At**: 2026-06-04T08:08:27Z
- **Labels**: `🟡 priority-medium`, `size-S`, `estimate-2`, `requirement`, `non-functional`

### Description
## Mục tiêu

Duy trì sự ổn định của hệ thống AutoWash Pro hoạt động liên tục đạt tỷ lệ 99.5% uptime.

## Căn cứ yêu cầu

Issue này được xác định dựa trên:
- Tài liệu SRS.md (NFR-05 Availability)
- File đề bài cô giáo giao (NFR-05: System Availability)
- Nhu cầu triển khai thực tế của project AutoWashCar

## Phạm vi công việc

- Triển khai endpoint kiểm tra sức khỏe hệ thống `/health` để giám sát trạng thái cơ sở dữ liệu và Tomcat.
- Thiết lập phương án khôi phục nhanh và khởi động lại dịch vụ khi phát hiện sự cố.

## Kết quả mong đợi

Sau khi hoàn thành issue này, hệ thống/tài liệu cần đạt được:

- Uptime hệ thống duy trì ở mức >= 99.5%, có hệ thống cảnh báo sớm khi gặp sự cố ngắt kết nối DB.
...
## [#52] NFR-04 LPR Processing: Thời gian nhận dạng biển số xe dưới 10 giây
- **URL**: https://github.com/NgaiLong49423/prj301-car-wash-system/issues/52
- **State**: open
- **Created At**: 2026-06-04T08:08:10Z
- **Labels**: `⚙️ Backend`, `🧪 Testing`, `🟢 priority-low`, `size-M`, `estimate-5`, `requirement`, `non-functional`

### Description
## Mục tiêu

Tích hợp và tối ưu hóa thời gian xử lý nhận diện biển số xe tự động (License Plate Recognition - LPR) trong vòng 10 giây.

## Căn cứ yêu cầu

Issue này được xác định dựa trên:
- Tài liệu SRS.md (NFR-04 LPR Processing)
- File đề bài cô giáo giao (NFR-04: LPR Processing Speed)
- Nhu cầu triển khai thực tế của project AutoWashCar

## Phạm vi công việc

- Xây dựng mô phỏng (Mock Service) hoặc API kết nối thiết bị camera LPR nhận biển số.
- Tối ưu hóa luồng xử lý dữ liệu biển số khi xe vào bãi rửa xe.
- Khớp biển số xe với cơ sở dữ liệu để phục vụ kiểm tra/check-in.

## Kết quả mong đợi

Sau khi hoàn thành issue này, hệ thống/tài liệu cần đạt được:

- Nhận diện biển số xe và trả về thông tin check-in trong vòng dưới 10 giây.
...
- NFR-04 LPR Processing

---

## [#51] NFR-03 Concurrent Users: Khả năng chịu tải tối thiểu 500 người dùng đồng thời
- **URL**: https://github.com/NgaiLong49423/prj301-car-wash-system/issues/51
- **State**: open
- **Created At**: 2026-06-04T08:07:53Z
- **Labels**: `🧪 Testing`, `🟡 priority-medium`, `size-M`, `estimate-3`, `requirement`, `non-functional`

### Description
## Mục tiêu

Đảm bảo hệ thống vận hành ổn định và không suy giảm hiệu năng khi có ít nhất 500 người dùng truy cập đồng thời.

## Căn cứ yêu cầu

Issue này được xác định dựa trên:
- Tài liệu SRS.md (NFR-03 Concurrent Users)
- File đề bài cô giáo giao (NFR-03: Concurrent Users)
- Nhu cầu triển khai thực tế của project AutoWashCar

## Phạm vi công việc

- Cấu hình Connection Pool cho SQL Server trong Tomcat 9 để quản lý tốt tài nguyên kết nối.
- Tối ưu hóa cấu hình Thread Pool của Tomcat.
- Thực hiện tải giả lập (Load Testing) bằng Apache JMeter với kịch bản 500 CCU.

## Kết quả mong đợi

Sau khi hoàn thành issue này, hệ thống/tài liệu cần đạt được:

- Hệ thống không bị treo hoặc lỗi kết nối (503) dưới tải 500 người dùng đồng thời.
...
## [#50] NFR-02 Booking Processing: Thời gian xử lý đặt lịch dưới 5 giây
- **URL**: https://github.com/NgaiLong49423/prj301-car-wash-system/issues/50
- **State**: open
- **Created At**: 2026-06-04T08:07:30Z
- **Labels**: `⚙️ Backend`, `🧪 Testing`, `🔴 priority-high`, `size-M`, `estimate-3`, `requirement`, `non-functional`

### Description
## Mục tiêu

Đảm bảo các giao dịch đặt lịch rửa xe và tính điểm tích lũy, áp dụng ưu đãi hoàn thành trong vòng 5 giây.

## Căn cứ yêu cầu

Issue này được xác định dựa trên:
- Tài liệu SRS.md (NFR-02 Booking Processing)
- File đề bài cô giáo giao (NFR-02: Booking Processing)
- Nhu cầu triển khai thực tế của project AutoWashCar

## Phạm vi công việc

- Tối ưu hóa Database Transactions trong BookingServlet.
- Đảm bảo cơ chế khóa dữ liệu (database locking) không gây nghẽn khi nhiều người đặt cùng lúc.
- Đo đạc thời gian xử lý khi chạy API đặt lịch.

## Kết quả mong đợi

Sau khi hoàn thành issue này, hệ thống/tài liệu cần đạt được:

- Lịch đặt được tạo và xác nhận kèm tính toán Loyalty trong vòng dưới 5 giây.
...
- NFR-02 Booking Processing

---

## [#49] NFR-01 Performance: Thời gian phản hồi hệ thống dưới 5 giây
- **URL**: https://github.com/NgaiLong49423/prj301-car-wash-system/issues/49
- **State**: open
- **Created At**: 2026-06-04T08:04:19Z
- **Labels**: `🧪 Testing`, `🟡 priority-medium`, `size-S`, `estimate-2`, `requirement`, `non-functional`

### Description
## Mục tiêu

Đảm bảo thời gian phản hồi của hệ thống đối với hầu hết các yêu cầu từ người dùng không quá 5 giây dưới tải bình thường.

## Căn cứ yêu cầu

Issue này được xác định dựa trên:
- Tài liệu SRS.md (NFR-01 Performance)
- File đề bài cô giáo giao (NFR-01: Response Time)
- Nhu cầu triển khai thực tế của project AutoWashCar

## Phạm vi công việc

- Tối ưu hóa các truy vấn SQL, hạn chế join quá nhiều bảng lớn không cần thiết.
- Áp dụng các kỹ thuật tối ưu hóa mã nguồn JSP/Servlet.
- Kiểm tra hiệu năng để đảm bảo thời gian phản hồi (chấp nhận 95% request dưới 5s).

## Kết quả mong đợi

Sau khi hoàn thành issue này, hệ thống/tài liệu cần đạt được:

- Tốc độ phản hồi trang web mượt mà, thời gian phản hồi thực tế <= 5 giây.
...
## [#48] FR-16 — Báo cáo doanh thu và lịch đặt dành cho Admin
- **URL**: https://github.com/NgaiLong49423/prj301-car-wash-system/issues/48
- **State**: open
- **Created At**: 2026-06-04T08:01:49Z
- **Labels**: `✨ Feature`, `🎨 Frontend`, `⚙️ Backend`, `🟡 priority-medium`, `size-M`, `estimate-3`
...
## [#47] FR-15 — Báo cáo thống kê khách hàng dành cho Admin
- **URL**: https://github.com/NgaiLong49423/prj301-car-wash-system/issues/47
- **State**: open
- **Created At**: 2026-06-04T08:01:21Z
- **Labels**: `✨ Feature`, `🎨 Frontend`, `⚙️ Backend`, `🟡 priority-medium`, `size-M`, `estimate-3`
...
## [#46] FR-14a — Gửi khuyến mãi nhắm mục tiêu theo hạng thành viên
- **URL**: https://github.com/NgaiLong49423/prj301-car-wash-system/issues/46
- **State**: open
- **Created At**: 2026-06-04T08:00:33Z
- **Labels**: `✨ Feature`, `🎨 Frontend`, `⚙️ Backend`, `🟡 priority-medium`, `size-M`, `estimate-5`
...
## [#45] FR-13 — Admin Loyalty Configuration: Cấu hình luật loyalty cho Admin
- **URL**: https://github.com/NgaiLong49423/prj301-car-wash-system/issues/45
- **State**: open
- **Created At**: 2026-06-04T07:59:56Z
- **Labels**: `✨ Feature`, `🎨 Frontend`, `⚙️ Backend`, `🗄️ Database`, `🟡 priority-medium`, `size-M`, `estimate-3`
...
## [#44] FR-12 — Điểm loyalty hết hạn sau 12 tháng
- **URL**: https://github.com/NgaiLong49423/prj301-car-wash-system/issues/44
- **State**: open
- **Created At**: 2026-06-04T07:59:35Z
- **Labels**: `✨ Feature`, `⚙️ Backend`, `🟡 priority-medium`, `size-M`, `estimate-3`
...
## [#43] FR-11 — Đổi điểm loyalty lấy ưu đãi
- **URL**: https://github.com/NgaiLong49423/prj301-car-wash-system/issues/43
- **State**: open
- **Created At**: 2026-06-04T07:58:48Z
- **Labels**: `✨ Feature`, `🎨 Frontend`, `⚙️ Backend`, `🔴 priority-high`, `size-M`, `estimate-3`
...
## [#42] FR-10a — Xét hạng thành viên tự động
- **URL**: https://github.com/NgaiLong49423/prj301-car-wash-system/issues/42
- **State**: open
- **Created At**: 2026-06-04T07:58:31Z
- **Labels**: `✨ Feature`, `⚙️ Backend`, `🗄️ Database`, `🔴 priority-high`, `size-M`, `estimate-3`
...
## [#41] FR-09 — Tích điểm tự động sau khi hoàn thành rửa xe
- **URL**: https://github.com/NgaiLong49423/prj301-car-wash-system/issues/41
- **State**: open
- **Created At**: 2026-06-04T07:51:08Z
- **Labels**: `✨ Feature`, `⚙️ Backend`, `🗄️ Database`, `🔴 priority-high`, `size-M`, `estimate-3`
...
## 8. Checklist nhiệm vụ cần hoàn thành (FR-09)

Vui lòng hoàn thành các đầu việc sau để giải quyết Issue này. Bạn có thể bấm trực tiếp vào ô vuông bên cạnh để đánh dấu đã hoàn thành:

- [x] **Bước 1: Cấu hình DB** - Chạy file `Database/database_v2.sql` để tạo unique index trên bảng `LoyaltyTransaction` nhằm chống cộng điểm trùng.
- [x] **Bước 2: Tạo DTO** - Tạo class `dto/LoyaltyTransactionDTO.java` đại diện cho bảng lịch sử điểm.
- [x] **Bước 3: Tạo DAO** - Viết `dao/LoyaltyDAO.java` chứa các phương thức insert giao dịch điểm và update điểm (`total_points`), tổng tiền (`total_spent_money`) trong bảng `Customer`.
- [ ] **Bước 4: Viết Service tích điểm** - Tạo class `service/LoyaltyService.java` xử lý tính điểm: `points = (total_price / 1000) * (1 + bonus_rate)` dựa trên hạng hiện tại của khách hàng.
- [ ] **Bước 5: Lưu giao dịch** - Đảm bảo phương thức tích điểm ghi nhận 1 transaction `Earned` liên kết đúng `booking_id` và `customer_id`.
- [ ] **Bước 6: Tích hợp vào Controller** - Mở `UserBookingHistoryServlet.java` ở nhánh check-in, gọi hàm tích điểm sau khi cập nhật trạng thái booking thành `Completed` thành công.
- [ ] **Bước 7: Cập nhật HTTP Session** - Đảm bảo cập nhật lại `totalPoints` của đối tượng User trong Session để đồng bộ hiển thị lên giao diện JSP ngay lập tức.
- [ ] **Bước 8: Gọi hàm xét hạng (FR-10a)** - Ở cuối luồng tích điểm thành công, gọi hàm xét lại hạng thành viên để kiểm tra nâng/hạ hạng.
- [ ] **Bước 9: Kiểm thử và xác nhận** - Thực hiện check-in một booking trên giao diện, mở trang Profile và Rewards để xác nhận điểm số đã tăng lên và hiển thị chính xác.
